...
inline int getBitFromLeft(unsigned int* bitmap, int index, int bitmapSize) {
    if(index >= bitmapSize) return -1;
    return (bitmap[index >> 5] & (1 << (index & 31))) > 0 ? 1 : 0;
}

inline bool setBitFromLeft(unsigned int* bitmap, int index, int bitmapSize, int bit) {
...
inline int getFirstBitFromLeft(unsigned int* bitmap, int bitmapSize, int bit) {
